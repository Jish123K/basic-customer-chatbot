# Import the social media monitoring library

from social_media_monitoring import SocialMediaMonitoring

# Define a function to get the latest social media posts for a company

def get_latest_social_media_posts(company_name):

    smm = SocialMediaMonitoring()

    posts = smm.get_latest_posts(company_name)

    return posts

# Update the `handle_inquiry()` function to use the `get_latest_social_media_posts()` function
...
def handle_inquiry(inquiry):
    ...
    # Get the latest social media posts for the company

    posts = get_latest_social_media_posts(config["company_name"])
    ...
    for post in posts:
        ...
        # If the post matches the inquiry text, use it as the response

        if post["text"].lower() == inquiry_text.lower():

            send_message(customer_id, post["message"])
...
# Update the `listen_for_inquiries()` function to use the `get_latest_social_media_posts()` function
